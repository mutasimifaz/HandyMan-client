...
const MyProfile = () => {
    ...
    useEffect(() => {
        fetch(`https://handyman-repairs.herokuapp.com/user/${user?.email}`)
            .then(res => res.json())
            .then(data => {
                setMyData(data)
            })
                ...
                <div>
                    <div className="justify-center flex object-center items-center lg:grid-cols-2 grid-cols-1 gap-3">
                        <div className='grid lg:grid-cols-2'>
                            ...
                            <div className='mt-8'>
                                <img className='mb-3 h-40 w-40 items-center justify-center flex' src={user?.photoURL} alt={user?.displayName}></img>
                                <h1 className='mb-3 text-md'>Name: {user.displayName}</h1>
                                <h1 className='mb-3 text-md'>Email: {user.email}</h1>
                                {myData.phone ? <p className='mb-3 text-md'>Phone: {myData.phone}</p> : <p className='mb-3 text-md'>Phone: N/A</p>}
                                {myData.location ? <p className='mb-3 text-md'>Location: {myData.location}</p> : <p className='mb-3 text-md'>Location: N/A</p>}
                                {myData.education ? <p className='mb-3 text-md'>Education: {myData.education}</p> : <p className='mb-3 text-md'>Education: N/A</p>}
                            </div>
                            </div>
