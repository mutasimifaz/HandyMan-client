...
const AllUserRow = ({ user, refetch, myEmail, setDeletingUser }) => {
    ...
    const makeAdmin = () => {
        fetch(`https://handyman-repairs.herokuapp.com/user/admin/${email}`, {
            method: 'PUT',
            headers: {
    ...
    const makeUser = () => {
        fetch(`https://handyman-repairs.herokuapp.com/users/admin/${email}`, {
            method: 'PUT',
            headers: {
                authorization: `Bearer ${localStorage.getItem('accessToken')}`
            }
    ...
    return (
        <tr className='bg-white'>
            ...
            <td className='w-12 h-12'>
                <img className='object-cover' alt={user?.user_name} src={user?.photo}></img>
            </td>
