...
const DeleteOrderModal = ({ deletingOrder, refetch, setDeletingOrder }) => {
  ...
  const handleDelete = () => {
    const url = `https://handyman-server-production.up.railway.app/orders/${_id}`;
    fetch(url, {
      method: "DELETE",
      headers: {
      ...
      <div className="modal modal-bottom sm:modal-middle">
        <div className="modal-box">
          ...
          <h3 className="font-bold text-lg text-red-600">
            Are you sure you want to cancel {name}'s order, id:{_id}
          </h3>
          <p className="py-4">
            You will not be able to recover item once you have deleted it!
          </p>
